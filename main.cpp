...
double Swap(int type, int r1, int r2, bool revert)
{
    ...
    if (slot2.stored == -1)
    {
        ...
        inst1.y = slot2.y;
        if (!revert)
            return netHPWL(inst1) - beforeSwap;
        else
            return -1.0;
    }
    else
...
double perturb()
{
    ...
    return Swap(randInst.type, randInst.rsrc, ResID, false);
}
...
void SA()
{
    ...
    int nAns, uphill, T = 1000000000, N = 100; // N: number of answer in T
    bestHPWL = originWL;
    do
    {
